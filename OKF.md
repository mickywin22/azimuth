...
| Reserved `log.md` = newest-first ISO-dated history | reserved filename | top-level `vault/log.md`, auto-generated from ingest days + brief changelogs by `scripts/build_log.py` (CI-guarded) | ✅ conformant (Tier-1) |
...
- **`index.md` not yet generated.** Folder listings use `README.md` rather than the reserved
  `index.md`. This is a cheap Tier-1 follow-up (a generator), tracked under KR `W26-Azimuth-USP`.
  The sibling `log.md` clause is now **done** — `vault/log.md` is generated by
  `scripts/build_log.py` and CI-guarded (`build_log.py --check`).
