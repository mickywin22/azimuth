...
format cannot produce. Hover a node to spotlight its links, scroll to zoom and drag to pan;
click a node to open its page. Held themes never appear.
...
spotlight its links &middot; drag a node to reposition &middot; click to open its page.</p>
...
let hoverId = null;
function hoverHL() {
  if (!hoverId || !ADJ[hoverId]) return null;
  const nodes = new Set([hoverId]), edges = new Set();
  for (const nb of ADJ[hoverId]) {
    nodes.add(nb); edges.add(hoverId+"|"+nb); edges.add(nb+"|"+hoverId);
  }
  return { nodes, edges };
}
const activeHL = () => HILITE || hoverHL();
...
function draw() {
  ...
  for (const e of E) {
    if (e.cross) continue;
    cx.strokeStyle = "#2a3a4d"; cx.lineWidth = lw(1); cx.globalAlpha = onEdge(e) ? 1 : 0.08;
    cx.beginPath(); cx.moveTo(e.s.x,e.s.y); cx.lineTo(e.t.x,e.t.y); cx.stroke();
  }
  for (const e of E) {
    if (!e.cross) continue;
    cx.strokeStyle = CROSS; cx.lineWidth = lw(2.6); cx.globalAlpha = onEdge(e) ? 0.9 : 0.08;
    cx.beginPath(); cx.moveTo(e.s.x,e.s.y); cx.lineTo(e.t.x,e.t.y); cx.stroke();
  }
  cx.globalAlpha = 1;
  ...
  for (const n of N) {
    ...
    if (n.id === hoverId) {  // a white focus ring on the hovered node
...
window.addEventListener("mousemove", ev => {   // drag-node / pan-background (capture anywhere)
  ...
  if (drag) { const [wx, wy] = toWorld(mx, my); drag.x = wx; drag.y = wy; moved++; }
  else { view.tx = pan.tx + (mx - pan.mx); view.ty = pan.ty + (my - pan.my); userView = true; moved++; }
});
cv.addEventListener("mousemove", ev => {       // hover spotlight + tooltip
...
cv.addEventListener("mouseleave", () => { hoverId = null; tip.style.display = "none"; });
...
cv.addEventListener("wheel", ev => {           // zoom toward the cursor
  ...
  view.tx = mx - wx*view.s; view.ty = my - wy*view.s; userView = true;
}, { passive: false });
function zoomBy(k) {
  const cx0 = CSSW/2, cy0 = CSSH/2, p = toWorld(cx0, cy0);
  view.s = Math.max(0.25, Math.min(6, view.s * k));
  view.tx = cx0 - p[0]*view.s; view.ty = cy0 - p[1]*view.s; userView = true;
}
...
if (zreset) zreset.addEventListener("click", () => { userView = false; resize(); });
