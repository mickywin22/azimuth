...
if (qa && qb) {
  ...
  document.getElementById("qclear").addEventListener("click", () => {
    HILITE = null; document.getElementById("qout").textContent = ""; mark();
  });
}
...
cv.addEventListener("mousemove", ev => {       // hover spotlight + tooltip
  ...
  if (newHover !== hoverId) mark();       // repaint the spotlight even when the layout is idle
  hoverId = newHover;
  if (hit) {
...
cv.addEventListener("mouseleave", () => { hoverId = null; tip.style.display = "none"; mark(); });
