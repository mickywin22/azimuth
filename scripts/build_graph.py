...
spotlight its links &middot; drag a node to reposition &middot; click to open its page
...
if (search) {
  ...
  const focusNode = () => {
    ...
    if (!q) { pinnedId = null; mark(); return; }
  ...
  search.addEventListener("change", focusNode);
  search.addEventListener("keydown", e => { if (e.key === "Enter") focusNode(); });
...
const activeHL = () => HILITE || neighHL(hoverId) || neighHL(pinnedId);
