...
> from the 2026-07-15 ingest; current cutoff: the 2026-08-31 pull, absorbing the
> 2026-08-21 through 2026-08-31 gap.)

## This week at a glance

- The abuse.ch / AbuseIPDB channel surfaced **exactly one active critical-severity
  indicator on both bookend pulls this cycle** — 2026-08-20 and 2026-08-31 — with the
  one-indicator critical tier holding flat across the gap ([[cyber-threats]]).
...
- **2026-08-31 pull:** one active threat indicator, type IP address, geolocated to
  **Romania** (RO), criticality CRITICAL, no malware family attributed — the same
  severity, type and country as the 08-20 reading, held steady over the cycle
  ([[cyber-threats]]).
- No daily pulls were absorbed for 2026-08-21 through 2026-08-30 this cycle (an 11-day
  curator gap) — azimuth compares only the two bookend pulls it actually holds and draws
  no rotation or trend inference across the days it did not observe ([[cyber-threats]]).
...
- The 2026-08-31 indicator carries the identical shape — type IP address, criticality
  CRITICAL, country RO, malwareFamily untagged — as the 2026-08-20 reading. This is a
  flat read across two bookend pulls, not a rotation: with the 08-21 through 08-30 daily
  pulls unabsorbed this cycle, azimuth has no basis to say whether the underlying
  indicator held the same host or simply landed on the same severity/type/country
  combination again ([[cyber-threats]]).
- Both readings' country field is geolocation of the indicator IP, not attribution of an
  actor; azimuth infers no actor, victim or campaign behind either pull's listed host
  ([[cyber-threats]]).
- azimuth records the indicator, its type, severity and geolocation exactly as the tracker
  scored them on each pull, and makes no trend claim from this two-point, gap-bridging
  comparison ([[cyber-threats]]).
