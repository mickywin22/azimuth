...
> from the 2026-07-15 ingest; current cutoff: 2026-08-20T07:01:59Z.)
...
- The surfaced indicator **rotated day-over-day** across the three pulls since the last brief —
  `170.239.205.222` (08-17, CO) → `195.178.110.232` (08-18, NL) → `69.5.169.180` (08-19, DE) →
  `80.94.95.242` (08-20, RO) — each a single critical malware-host at AbuseIPDB score 100.
  totalCount **eased** from **1,023** (08-17) to **984** (08-20), a net **-39** over three days,
  slowing each day (-27, then -8, then -4) — the daily add/retire churn of the curated critical
  slot, not a directional trend ([[cyber-threats]]).
...
- The surfaced IP rotated day-over-day across the window since the last brief —
  `170.239.205.222` (08-17, CO) → `195.178.110.232` (08-18, NL) → `69.5.169.180` (08-19, DE) →
  `80.94.95.242` (08-20, RO) — each carrying the same CRITICAL severity and score:100 tag as
  assigned by the upstream tracker. The origin geolocation moved CO → NL → DE → RO; this is a
  geolocation of the indicator IP, not an actor attribution ([[cyber-threats]]).
- The feed's pagination totalCount moved 1,023 (08-17) → 996 (08-18) → 988 (08-19) → 984
  (08-20), a net -39 three-day decline that eased each day (-27, then -8, then -4) — the
  add/retire churn of the curated feed, not a confirmed directional build. The single-slot
  surface against a ~980-1,000-count feed remains the known honest-scope posture of the
  WorldMonitor curated channel ([[cyber-threats]]).
...
- 2026-08-20 — daily-ingest synthesis (2026-W34): absorbed the 2026-08-18 through 2026-08-20 abuse.ch pulls; the surfaced critical malware-host rotated every day — 195.178.110.232 (08-18, NL), 69.5.169.180 (08-19, DE), 80.94.95.242 (08-20, RO) — after 170.239.205.222 (08-17, CO), each AbuseIPDB score 100, CRITICAL, no malware family; totalCount moved 1,023 → 996 → 988 → 984 (net -39, a steady three-day retirement that eased each day: -27, -8, -4); origin geolocation CO → NL → DE → RO (geolocation, not attribution). One-indicator critical tier held. Observed-only framing held ([[cyber-threats]]).
